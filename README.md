...
[Raspberry Pi 3 Model B] (以下Raspberry Piと称する)に温度センサーを取り付け、取得された温度データを[AWS IoT Device SDK for Python]を利用し、AWS IoTへ送信する。送信された温度データをElasticsearchで蓄積し、Kibanaで統計結果を確認する。

## [Raspberry Pi]に温度センサーを取り付ける

### 配線

下図に示すように温度センサー[DA18B20]の３本の足を[Raspberry Pi]につなぐ。
...
- [Raspberry Pi]からの送信、及びサーバ側の受信を確認する
...
2. AWS管理者からIAMユーザーのアクセスポリシーを作成する。
    作成されたポリシーのARNが出力されるので、それを控えておく。

        $ aws iam create-policy \
            --policy-name iotDevPolicy \
            --policy-document file://json/iam-dev-policy.json
        # 出力されたポリシーのARNを控え、以降の<IAMユーザーポリシーのARN>に書き換えてください
...
1. Elasticsearchドメインの作成（作成完了まで約１０分間かかる。）
